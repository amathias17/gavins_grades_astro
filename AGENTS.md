...
- Homepage payout dashboard renders a compact server-rendered MissingAssignmentsSummary below the purchase-equivalent label, showing assignment, class, due date, and points; an empty list shows an all-clear message.
...
- The mobile payout dashboard reserves at least 3rem of top breathing room plus any iOS safe-area inset.
...
- Phase 1 points foundation: src/utils/points.ts derives current-marking-period points from scraper assignments and grades. It awards actual earned points, +2 per unique full-credit assignment, and +10 per current numeric A grade; incomplete work is an opportunity with no penalty. Homepage migration is intentionally deferred until phase confirmation.
