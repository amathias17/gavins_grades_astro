...
- Homepage shows the current DBZ power-level badge, while /badges provides the full seven-badge collection with local thresholds and protected quest-point state. Badge artwork and future character metadata are hydrated at build time through src/utils/dragonBallApi.ts from the Dragon Ball API; API failures render the existing placeholder, and local public/badges images are not required. /badges also displays dynamic collected and remaining badge counts with collection progress derived from unlocked badge state.
