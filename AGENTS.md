...
- Homepage shows the current DBZ power-level badge, while /badges provides the seven-badge collection in this order: Yamcha, Piccolo, Gohan, Vegeta, Goku, Gogeta, and Goku Ultra Instinct. Local thresholds and protected quest-point state remain authoritative. Badge artwork and future character metadata are hydrated at build time through src/utils/dragonBallApi.ts from the Dragon Ball API; API failures render the existing placeholder, and local public/badges images are not required. Badge artwork uses uniform 4:5 portrait framing with a consistent top-weighted zoomed crop so character faces remain visible. /badges also displays dynamic collected and remaining badge counts with collection progress derived from unlocked badge state.
- Homepage current badge panel has a prominent OPEN BADGE ROOM CTA and makes the full current badge card a keyboard-accessible link to /badges. Locked Badge Room cards conceal character names, initials, final labels, and API artwork behind a CSS mystery silhouette until unlocked; they expose only level, lock state, and points required.
- Unlocked Badge Room cards also show Dragon Ball API Affiliation, Base KI, and Total KI stats; locked cards omit these stats. Badge artwork is visible by default on first render, with the existing placeholder retained for image failures, and the homepage current badge card has extra heading-to-card spacing.
- Homepage DATA UPDATED now appears directly below the points encouragement line; the current badge card no longer displays CURRENT BADGE, and its panel no longer displays CURRENT CHARACTER while retaining an accessible section label.
- Homepage marking-period card shows DATA UPDATED using the newest valid grade or detailed-assignment source timestamp, formatted in Eastern Time; the protected quest ledger timestamp is not used as the data freshness indicator.
...
- Assignment breakdown uses reconciled PointAssignment statuses server-rendered as COMPLETED, MISSING, or NOT GRADED; its summary separates assignment points, period possible, ready-to-earn, awaiting-grade, bonuses, and total earned points.
- ALL OPPORTUNITIES uses responsive cards with wrapping assignment/class/date metadata and visually separated status and point values, including distinct missing and not-graded accents.
...
- Class-detail assignment rows reconcile normalized class, assignment, and due-date matches from missing_assignments.json, overriding confirmed missing work to MISSING with 0/total points while preserving other scraped statuses.
...
- Scraper updates: scraper/enhanced-scraper.cjs now clicks sf_expander, exhausts all "Next" pagination links (moreAssignmentsEvents_*) per class, detects the active quarter from Skyward's highlighted grade cell, filters tagged assignments to that quarter, opens showAssignmentInfo dialogs, and pulls Points Earned/Total Points/Due Date via document XPaths with dialog DOM fallbacks before closing via sf_DialogClose. If Skyward has not highlighted a quarter, assignments are retained instead of silently discarded and the extracted grade is preserved as current_grade. Date fields are normalized to drop numeric-only or label-only values. Runtime sped up with headless launch, no slowMo, shorter waits, and dialog-visible/hidden waits instead of fixed sleeps. Output remains detailed-grades.json (per class with assignment weights/points) plus raw. Graded assignments cache to scraper/detailed-grades-cache.json (gitignored) to skip re-scraping; "* out of x" are kept as 0/out-of-X and left uncached until graded; raw output now includes class names plus due dates; organized output omits classes with no assignments. ClassId mapping now also uses assignment row group-child/group-parent attributes with classDesc lookups to restore className/period/teacher after expansion.
- Scraper supports SKYWARD_MAX_ASSIGNMENTS to cap assignment detail scraping for quick checks.
- GitHub Actions grade scraping accounts for Eastern daylight saving time by triggering at both UTC candidates and running only during 7:00 AM, 3:00 PM, or 8:00 PM America/New_York hours; GitHub schedule delays can still affect exact execution time.
