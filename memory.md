...
- Scraper fix (12/21/2025): extractAssignmentDetails now clicks by data-aid/data-gid instead of index; organizeByClass groups by classId first, yielding 9 separate classes. Known issue (gavins_grades_astro-orf): classIdMap DOM traversal fails post-expansion, so className/period/teacher are empty (classId used as fallback). Assignments correctly separated but lack metadata.
