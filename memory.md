...
- Scraper and credentials: scraper/ contains Playwright scripts (skyward-scraper.cjs, investigate-assignments.cjs, enhanced-scraper.cjs) for Skyward; enhanced-scraper exhausts all moreAssignmentsEvents_* pagination links, filters Q2, opens showAssignmentInfo to capture Points Earned/Total Points/Weight, and closes sf_DialogClose. Adds graded-assignment cache at scraper/detailed-grades-cache.json (gitignored) to avoid re-scraping; “* out of x” stays uncached with 0/total so new scores re-fetch. Raw output now carries class names plus assign/due dates. .claude/settings.local.json stores allowed commands and SKYWARD_USERNAME/PASSWORD values. Avoid committing secrets and be cautious running scraper commands. Debug/test artifacts cleaned (dist/, playwright-report/, test-results/, debug images/html).
