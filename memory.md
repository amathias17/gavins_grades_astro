...
- Scraper supports SKYWARD_MAX_ASSIGNMENTS to cap assignment detail scraping for quick verification runs. The enhanced scraper detects the active quarter from Skyward's highlighted grade cell, filters only explicitly tagged assignments from other quarters, and retains assignments when no quarter is highlighted; grades are written to the matching q1/q2/q3/q4 field.
