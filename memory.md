...
- Payout display uses Orbitron and DM Mono loaded from Google Fonts. The current points total is a static server-rendered value with a milestone progress bar; reduced-motion users retain the same non-animated information.
- The feedback button still opens a full-screen accessible modal containing "lol turn in your assignments." with Close, Escape, backdrop click, and focus restoration behavior.
