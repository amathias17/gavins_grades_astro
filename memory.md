...
- Scraper and credentials: scraper/ contains Playwright scripts (skyward-scraper.cjs, investigate-assignments.cjs) for Skyward; .claude/settings.local.json stores allowed commands and SKYWARD_USERNAME/PASSWORD values. Avoid committing secrets and be cautious running scraper commands. Debug/test scripts and raw scrape outputs removed for cleanliness.
- Commands: npm run dev/build/preview; npx playwright test (installs browsers per playwright.config.ts). Data files include non-ASCII artifacts, so prefer ASCII in new edits. Build/test artifacts and debug screenshots are cleaned (dist/, playwright-report/, test-results/, debug images/html).
