...
- Scraper and credentials: scraper/ contains Playwright scripts (skyward-scraper.cjs, investigate-assignments.cjs, enhanced-scraper.cjs) for Skyward; enhanced-scraper exhausts all moreAssignmentsEvents_* pagination links, filters Q2, opens showAssignmentInfo, and pulls Points Earned/Total Points/Assign Date/Due Date via document XPaths with dialog DOM fallbacks before closing sf_DialogClose, normalizing date fields to drop numeric-only or label-only values. Runtime sped up with headless launch, no slowMo, shorter waits, and dialog-visible/hidden waits. Adds graded-assignment cache at scraper/detailed-grades-cache.json (gitignored) to avoid re-scraping; "* out of x" stays uncached with earned=0/total preserved so new scores re-fetch. Raw output now carries class names plus assign/due dates, and organized output omits classes with no assignments. .claude/settings.local.json stores allowed commands and SKYWARD_USERNAME/PASSWORD values. Avoid committing secrets and be cautious running scraper commands. Debug/test artifacts cleaned (dist/, playwright-report/, test-results/, debug images/html).
