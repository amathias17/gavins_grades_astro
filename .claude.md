...
**SKIP these tools:**
- ❌ `deepwiki` for general concepts (use only if truly needed)
- ⚠️ `sequential-thinking` - Keep to 3-4 thoughts max, not 8+

**USE these efficiently:**
- ✅ `Glob` + `Read` for repo exploration (fast, local)
- ✅ `Grep` for finding patterns
- ✅ `astrodocs` only when Astro-specific patterns needed
- ✅ Direct implementation after basic understanding
...
- ✅ You know where the code goes
- ✅ You've seen 1-2 similar components
- ❌ Don't need to understand every edge case upfront
...
│   ├── *.astro         # Server-rendered, can have <script> for client-side
│   └── Use retro gaming theme: #00ff00, #000, borders
├── pages/              # Routes (file-based routing)
│   ├── index.astro     # Home page (/)
│   ├── classes/
│   │   └── [classId].astro  # Dynamic routes
├── types/              # TypeScript interfaces
│   └── grades.ts       # Main data models
├── utils/              # Helper functions (pure, testable)
├── data/               # JSON data files
│   ├── grades.json     # Main grades data (auto-updated)
│   └── missing_assignments.json
└── styles/             # Global CSS
...
**Important:**
- ⚠️ Most classes DON'T have `assignments` array
- ⚠️ Grades come pre-calculated from scraper (no calculation logic in app)
- ⚠️ `class_id` may be undefined, use `period` instead
...
- [ ] **Document concisely:**
  ...
  - [ ] ❌ Skip lengthy feature docs unless requested
...
- ✅ README.md updates for major features
- ❌ Skip verbose feature specs unless user requests
- ❌ Skip architecture docs for small changes
...
- ❌ Don't over-research before coding
- ❌ Don't add new runtime dependencies without asking
- ❌ Don't create elaborate docs for simple features
- ❌ Don't duplicate existing code (reuse `getGradeColor`, etc.)
- ❌ Don't break the retro gaming aesthetic
- ❌ Don't assume all classes have `assignments` data

## ✅ What TO Do

- ✅ Start coding after basic understanding (70% confidence)
- ✅ Reuse existing design patterns and colors
- ✅ Keep TypeScript strict (no `any`)
- ✅ Make utility functions pure (testable)
- ✅ Add E2E tests for critical paths
- ✅ Use `<script>` tags for simple client-side logic
- ✅ Match the retro gaming theme exactly
